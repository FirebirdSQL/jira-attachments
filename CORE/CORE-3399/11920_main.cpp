...
const char dbFileName[] = "txtest.fdb";

const ISC_SCHAR dpb[] = {
	isc_dpb_version1, 
	isc_dpb_user_name, 6, 'S', 'Y', 'S', 'D', 'B', 'A', 
	isc_dpb_password, 8, 'm', 'a', 's', 't', 'e', 'r', 'k', 'e'
};
...
int main()
{
	...
	if (!isc_attach_database(status, sizeof(dbFileName), dbFileName, &hDB, sizeof(dpb), dpb))
		isc_drop_database(status, &hDB);

	if (isc_create_database(status, sizeof(dbFileName), dbFileName, &hDB, sizeof(dpb), dpb, 0))
...
void runSeries(isc_db_handle &hDB)
{
	...
	for (const char** tx1 = txKinds; tx1 < lastTx; tx1++)
		for (const char** tx2 = txKinds; tx2 < lastTx; tx2++)
		{
			...
			AttachHolder hDB2(dbFileName, sizeof(dpb), dpb);
