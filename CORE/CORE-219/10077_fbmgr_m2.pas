...
const
  ...
  RegKey  = 'Software\FirebirdSQL\Firebird\CurrentVersion';
  ...
  KRunGN  = 'FirebirdGuardian';
  KRunFN  = 'FirebirdServer';

  KRunG   = 'fbguard.exe';
  KRunF   = 'fbserver.exe';

  KService = 'FirebirdServer';
  KGService= 'FirebirdGuardian';

  KCName  = 'FB_Server';
  KWName  = 'Firebird Server';

  KSName  = 'Firebird Server';
  KSGName = 'Firebird Guardian Service';

  FBMgr_VersionNum   = '1.501';
...
{$IFDEF ENGLISH_VERSION}
  ...
  FB_NotInstalled = 'Please install Firebird Manager (fbmgr.exe) in your Firebird /bin directory';
...
begin
  ...
  // is fbmgr.exe in <root> or in 'bin' folder?
  fbroot := ExtractFilePath(Application.ExeName);
  i := pos('\BIN\',uppercase(fbroot));
  if (i > 0) then System.Delete(fbroot,i+1,4);
  if not fileExists(fbroot + 'bin\fbserver.exe') then
    begin
  ...
  // open the Firebird Registry keys : write entries if they do not exist
  MyReg := TRegistry.Create;
  MyReg.RootKey := RootKey;
  if not MyReg.OpenKey( RegKey ,False) then
    begin
    MyReg.OpenKey(RegKey,true);
    MyReg.WriteString('RootDirectory',fbroot);
    MyReg.WriteString('ServerDirectory',fbroot+'bin\');
    end;
