...
begin
  ...
  i := pos('\BIN\',uppercase(s)); // is executable in <root> or in 'bin' folder?
  if (i > 0) then System.Delete(s,i+1,4);
  if not MyReg.OpenKey( RegKey ,False) then
    begin
    MyReg.OpenKey(RegKey,true);
    MyReg.WriteString('RootDirectory',s);
    MyReg.WriteString('ServerDirectory',s+'bin\');
    end;
